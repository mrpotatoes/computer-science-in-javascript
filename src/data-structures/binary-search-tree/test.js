 ...
 *
 * A test conversion of https://github.com/louischatriot/node-binary-search-tree/tree/master/lib
 */
...
class BinarySearchTree {
  ...
  // Check that all nodes (incl. leaves) fullfil condition given by fn test is a function
  // passed every (key, data) and which throws if the condition is not met
  ...
  // Check that the core BST properties on node ordering are verified. Throw if they aren't
  checkNodeOrdering () {
    const self = this
  ...
  getLowerBoundMatcher (query) {
    ...
    if (query.hasOwnProperty('$gt') && query.hasOwnProperty('$gte')) {
      if (self.compareKeys(query.$gte, query.$gt) === 0) {
        return (key) => self.compareKeys(key, query.$gt) > 0
      }

      if (self.compareKeys(query.$gte, query.$gt) > 0) {
        return (key) => self.compareKeys(key, query.$gte) >= 0
      }
      return (key) => self.compareKeys(key, query.$gt) > 0
    }
  ...
  // Delete the current node if it is a leaf. Return true if it was deleted
  deleteIfLeaf () {
  ...
  // Delete the current node if it has only one child. Return true if it was deleted
